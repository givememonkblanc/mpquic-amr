#!/usr/bin/env bash
# run_record_stream.sh — record a playable video from the received MJPEG stream
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
ROOT_DIR="${ROOT_DIR:-$(cd "${SCRIPT_DIR}/.." && pwd)}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d-record-stream-%H%M%S)}"
...
exec env \
  ...
  bash "${SCRIPT_DIR}/run_preview_stream.sh"
