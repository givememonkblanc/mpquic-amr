...
## 집계 (30개 코멘트) — 2026-07-13 A–F 텍스트 패스 적용 후

산출물: `docs/paper_revision/Com-0420_revised.docx` (전문 텍스트: `Com-0420_revised.txt`). 원본 무변경.

| 상태 | 개수 | 항목 |
|---|---|---|
| ✅ 원고 텍스트 반영완료 | **20** | R1.1–R1.9, R2.1–R2.6, R3.1, R3.3, R3.5, R3.10(표기·수식), R3.7(용어) |
| ✅텍스트 + 🔬수치대기 | 8 | R1.10, R2.7, R2.9, R2.10, R3.4, R3.6, R3.8, R3.9 (문장 틀·정의 완료, `【NUM】` 53곳 자리표시) |
| 🔬 순수 실험수치 | 2 | R2.8(핸드오버 횟수 행), R3.2(완화 문구는 반영, 수치 대기) |
| 📝 figure 재작도 | 1 | R3.10 figure 저해상(Fig1–9 재생성, 별도) |
| ⛔ 블록 | 1 | R1.10 **검증**(ssh_edge 버그 선수정) — 논문 텍스트·코드는 완료 |
| 🔶 확인 필요 | 2 | `【CONFIRM】` 백업회선 5G 종류 / AP배치·로봇속도 |

**한 줄 요약**: 텍스트로 끝낼 수 있는 **28/30이 revised.docx에 반영 완료**. 남은 것 = ① N=10 매트릭스로 `【NUM】` 53곳 채우기, ② `【CONFIRM】` 2건 사실 확인, ③ Fig 재작도. 편집 이력: `paper_revision/apply_edits.py`.
